...
/*
Permet de séparer un string selon un caractère délimitant
Tiré de : http://stackoverflow.com/a/236803/6316091
*/
...
/*
* C1. Crée le graphe à partir d'un fichier
*/
...
/*
* C3. Extraire le graphe des points accessibles à un véhicule
*/
...
/*
* C3 (suite). Fonction récursive qui extrait le graphe disponible au véhicule
*/
vector<pair<Sommet*, int>> Graphe::extraireVoisinsDeSommet(Sommet* sommet, Vehicule vehicule) const {
	if (sommet->peutRecharger(vehicule.getCarburant())) //on recharge le véhicule partout où c'est possible
		vehicule.recharger();
	...
	for (auto i = voisinsPossibles.begin(); i != voisinsPossibles.end(); i++) {
		//if (!(*i).first->voisinsPossiblesExtraits()) { //voir rapport difficultés rencontrées
			vehicule.setAutonomieActuelle(autonomieActuelle - (*i).second);
			//(*i).first->setVoisinsPossiblesExtraits(true);
			vector<pair<Sommet*, int>> voisinExtrait = extraireVoisinsDeSommet((*i).first, vehicule); //on appelle récursivement
...
/*
* C4. Détermine le plus court chemin pour faire le trajet du départ à la destination
* Implémente l'algorithme de Dijkstra, inspiré de : https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm?oldformat=true#Pseudocode
*/
void Graphe::plusCourtChemin(Sommet* sommetDepart, Sommet* sommetDestination, Vehicule& vehicule) {
	...
	vector<pair<Sommet*, pair<int, Sommet*>>> sommetsEtPoids; //format : <pair<sommetActuel, pair<poidsDuSommetActuel, sommetPrécédent>>
	...
	while (sommetsDisponibles.size() != 0) { //on continue tant qu'il nous reste des sommets non testés
		...
		//on itère sur la liste de voisins du plus petit noeud
	...
	if (poidsFinal.second.first < numeric_limits<int>::max()) {
		//on a trouvé un chemin vers le sommet de destination
...
//Trouve un sommet dans un vecteur pair<Sommet*, pair<int, Sommet*>> passé en paramètre
...
//Trouve un sommet dans un vecteur de Sommet* passé en paramètre
...
int Graphe::calculerAutonomieRestante(const Vehicule& vehicule, vector<pair<Sommet*, pair<int, Sommet*>>>& vecteur, Sommet* sommetDepart, Sommet* sommetDestination) const {
	...
	return autonomieDepart - distanceTotale; //le véhicule n'a tout simplement pas rechargé durant son trajet
}
